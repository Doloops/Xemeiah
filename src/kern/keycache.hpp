...
{
    __INLINE LocalKeyId
    KeyCache::getFromCache (const char* keyName)
    {
#if PARANOID
    ...
    __INLINE KeyId
    KeyCache::getKeyId (NamespaceAlias& nsAlias, const char* keyName, bool create)
    {
        NamespaceId nsId = 0;
    ...
    __INLINE KeyId
    KeyCache::getKeyId (NamespaceAlias& nsAlias, const String& keyName, bool create)
    {
        return getKeyId(nsAlias, keyName.c_str(), create);
    }

    __INLINE const char*
    KeyCache::getKey (NamespaceAlias& nsAlias, KeyId keyId)
    {
    ...
    __INLINE const char*
    KeyCache::getLocalKey (LocalKeyId localKeyId)
    {
        return localKeyMap.get(localKeyId);
    }

    __INLINE const char*
    KeyCache::getKey (LocalKeyId prefixId, LocalKeyId localKeyId)
    {
