...
namespace Xem
{
    ...
    Store::Store () :
            keyCache(*this), volatileAreasMutex("Volatile Areas")
    {
    ...
    void
    Store::releaseDocument (Document* doc)
    {
        Log_Store("releaseDocument(doc=%p)\n", doc);
    ...
    Store::housewife ()
    {
        ...
        Info("\tCurrent VolatileAreas : %llu MBytes (%llu areas)\n",
             (volatileAreas.size() * DocumentAllocator::AreaSize) >> 20, (__ui64) volatileAreas.size());
    }
}
;
