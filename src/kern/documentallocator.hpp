...
namespace Xem
{
  __INLINE bool DocumentAllocator::isWritable ()
  {
#if PARANOID
  ...
  __INLINE FreeSegmentsLevelHeader* DocumentAllocator::getFreeSegmentsLevelHeader ( AllocationProfile allocProfile )
  {
#if PARANOID
  ...
  __INLINE __ui64 DocumentAllocator::alignSize ( __ui64 size )
  {
   ...
   */
   __INLINE bool DocumentAllocator::freeSegment ( SegmentPtr segPtr, __ui64 size )
  {
    AllocationProfile allocProfile = getAllocationProfile ( segPtr );
    return freeSegment ( segPtr, size, allocProfile );
  }

  __INLINE bool DocumentAllocator::freeSegment ( SegmentPtr segPtr, __ui64 size, AllocationProfile allocProfile )
  { 
#if 0
