...
namespace Xem
{
    const char* JournalOperationLabel[] =
        { "NoOp", "InsertChild", "AppendChild", "InsertBefore", "InsertAfter", "UpdateTextNode", "Remove",
                "UpdateAttribute", "DeleteAttribute", "UpdateBlob", "BuildMeta", "Invalid : LastOperation" };
    ...
    void
    JournaledDocument::appendJournalItem (SegmentPtr journalItemPtr, JournalItem* journalItem)
    {
    ...
    void
    JournaledDocument::appendJournal (ElementRef& baseRef, JournalOperation op, ElementRef& altRef,
    ...
    void
    JournaledDocument::applyJournal (XProcessor& xproc, JournaledDocument& source)
    {
        SegmentPtr journalItemPtr = source.getJournalHead().firstJournalItem;
        Log_Journal ("Initial journalItemPtr at %llx\n", journalItemPtr);
        while (journalItemPtr)
        {
            ...
            catch (Exception * e)
            {
                detailException(
                        e,
                        "Could not apply journal op=%d:'%s', because baseElement %llx does not exist in target Document.\n",
                        op, JournalOperationLabel[op], journalItem->baseElementId);
            ...
            if (journalItem->altElementId && op != JournalOperation_Remove)
            {
                ...
                catch (Exception* e)
                {
                    ...
                    detailException(
                            e,
                            "Could not apply journal op=%d:'%s', because altElement %llx does not exist in source Document.\n",
            ...
            switch (op)
            {
                ...
                case JournalOperation_UpdateAttribute:
                {
                    ...
                    else
                    {
                        ...
                        for (AttributeRef sourceAttr = sourceAltElement.getFirstAttr(); sourceAttr; sourceAttr =
                                sourceAttr.getNext())
                        {
                        ...
                        else
                        {
                            throwException(
                                    Exception,
                                    "[JOURNAL-APPLY] Attribute not found ! op=%x, attr=%x (%s), next=0x%llx, base=0x%llx, alt=0x%llx, base=%s\n",
                                    journalItem->op, journalItem->attributeKeyId,
                                    getKeyCache().dumpKey(journalItem->attributeKeyId).c_str(),
                                    journalItem->nextJournalItem, journalItem->baseElementId, journalItem->altElementId,
                                    sourceAltElement.generateVersatileXPath().c_str());
                        }
                    }
                    break;
    ...
    void
    JournaledDocument::duplicateElement (XProcessor& xproc, ElementRef& target, ElementRef& source)
    {
