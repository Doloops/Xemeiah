...
{
    BranchRevId
    BranchManager::parseBranchRevId (const char* branchRevStr)
    {
    ...
    static const char* documentFlagStrings[] =
        { "not-set", "read", "explicit-read", "write", "reuse-write", "as-revision", "unmanaged", "reset-revision",
                "follow-branch", NULL };

    DocumentOpeningFlags
    BranchManager::parseDocumentOpeningFlags (const String& flagsStr)
    {
    ...
    const char*
    BranchManager::getDocumentOpeningFlagsName (DocumentOpeningFlags flags)
    {
    ...
    Document*
    BranchManager::openDocument (const String& branchName, const String& flagsStr, RoleId roleId)
    {
        BranchId branchId = 0;
    ...
    Document*
    BranchManager::openDocument (BranchId branchId, RevisionId revisionId, DocumentOpeningFlags flags)
    {
    ...
    static const char* branchFlagNames[] =
        { "must-fork-before-update", "may-index-name-if-duplicate", "auto-forked", "auto-merge", "may-not-be-xupdated",
                "can-fork-at-reopen",
                NULL };
