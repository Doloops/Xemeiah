...
namespace Xem
{
  ...
  void* __callStopServiceManagerThread (void* arg );

  static void __callStopServiceManager ( void* arg )
  {
    pthread_t threadid;
    pthread_attr_t attr;
  ...
  void ServiceManager::stopServiceManager ( )
  {
    void* arg = (void*) this;
  ...
  void xemServiceSIGINTHandler ( int sig, siginfo_t* info, void* ucontext )
  {
  ...
  static void registerSignalSIGINT ()
  {
  ...
  void ServiceManager::registerService ( const String& serviceName, Service* service )
  {
  ...
  void ServiceManager::unregisterService ( const String& serviceName )
  {
    ...
    throwException ( Exception, "Could not unregister service '%s' : status=%s\n",
        serviceName.c_str(), service->getState().c_str() );
  }
  ...
  bool ServiceManager::areAllServicesStopped ()
  {
    bool remains = false;
    for ( ServiceMap::iterator iter = serviceMap.begin () ; iter != serviceMap.end () ; iter++ )
      {
        ...
        if ( ! service->isStopped () )
          {
            Info ( "\tService '%s' still not stopped, status=%s.\n", iter->first.c_str(), iter->second->getState().c_str() );
  ...
  void ServiceManager::broadcastStopService ()
  {
    for ( ServiceMap::iterator iter = serviceMap.begin () ; iter != serviceMap.end () ; iter++ )
      {
        Service* service = iter->second;
  ...
  void ServiceManager::doStopServiceManager ()
  {
  ...
  void ServiceManager::waitTermination ()
  {
    usleep ( 1 );
