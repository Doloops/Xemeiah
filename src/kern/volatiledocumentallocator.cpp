...
namespace Xem
{
  ...
  {
    Log_VolDocAlloc ( "Deleting VolatileDocumentAllocator at %p, areas=%p\n", this, areas );
    if ( areas )
      {
        for ( __ui64 idx = 0 ; idx < areasAlloced ; idx ++ )
          {
            void* area = areas[idx];
            if ( ! area ) continue;
            Log_VolDocAlloc ( "[VOLATILECONTEXTUNMAP] idx=0x%llx, area=%p\n", idx, area );
   ...
   */
  void VolatileDocumentAllocator::authorizePageWrite ( RelativePagePtr relPagePtr )
  {
  }

  void VolatileDocumentAllocator::mapDirectPage ( RelativePagePtr relPagePtr )
  {
    __ui64 areaIdx = relPagePtr >> InAreaBits;
    mapArea ( areaIdx );
  }

  void VolatileDocumentAllocator::mapArea ( __ui64 areaIdx )
  {
    ...
    if ( areas[areaIdx] == NULL )
      {
        ...
        areas[areaIdx] = getStore().getVolatileArea ();
	      Log_VolDocAlloc ( "[VOLATILEPAGEMAP] this=%p, areaIdx=0x%llx -> MMapped to %p, areasAlloced=0x%llx\n", 
	          this, areaIdx, areas[areaIdx], areasAlloced );
      }
  }

  RelativePagePtr VolatileDocumentAllocator::getFreeRelativePages ( __ui64 askedNumber,  __ui64& allocedNumber, AllocationProfile allocProfile )
  {    
  ...
  AllocationProfile VolatileDocumentAllocator::getAllocationProfile ( SegmentPtr segmentPtr )
  {
    return 0;
  }
