...
namespace Xem
{
  ...
  void KeyCache::unloadKeys ()
  {
    keysBucket.clear ();
    namespaceBucket.clear ();
  }

  LocalKeyId KeyCache::buildNamespacePrefix ( NamespaceAlias& nsAlias, NamespaceId nsId )
  {
    char prefixKey[256];
    for ( __ui32 id = 0 ; id < 256 ; id ++ )
      {
        ...
        if ( nsAlias.getNamespaceIdFromPrefix ( prefixId ) )
          {
            continue;
            NotImplemented ( "For namespaceId=%x, could not assign key '%s' (id %x) because it is already mapped to namespaceId=%x\n",
  ...
  KeyId KeyCache::buildNamespaceDeclaration ( LocalKeyId prefixId )
  {
    return getKeyId ( getBuiltinKeys().xmlns.ns(), prefixId );
  }

  NamespaceId KeyCache::getNamespaceId ( const char* namespaceURL )
  {
    if ( ! *namespaceURL ) return 0;
  ...
  const char* KeyCache::getNamespaceURL ( NamespaceId nsId )
  {
    if ( nsId == 0 ) return "";
    return namespaceMap.get ( nsId );
  }

  void KeyCache::parseKey ( const String& name, LocalKeyId& prefixId, LocalKeyId& localKeyId )
  {
    ...
    if ( ! localKey )
      {
        ...
        if ( localKeyId == 0 ) throwException ( InvalidKeyException, "Empty key !\n" );
        return;
      }
  ...
  KeyId KeyCache::getKeyId ( NamespaceId nsId, const char* keyName, bool create )
  {
  ...
  KeyId KeyCache::getKeyIdWithElement ( const ElementRef& _parsingFromElementRef, const String& keyName, bool useDefaultNamespace )
  {
        ...
        if ( ! nsId )
          {
            Error ( "Could not resolve namespace for prefix : '%s', at element '%s'\n",
                keyName.c_str(), parsingFromElementRef.generateVersatileXPath().c_str() );
            throwException ( UnresolvableNamespaceException, "Could not resolve namespace for prefix : '%s', at element '%s'\n",
                keyName.c_str(), parsingFromElementRef.generateVersatileXPath().c_str() );
          }
      }
    return getKeyId(nsId, localKeyId);
  ...
  String KeyCache::dumpKey ( KeyId keyId )
  {
    String key;
  ...
  void KeyCache::forAllKeys ( void (*func) ( void* arg, LocalKeyId localKeyId, const char* keyName ), void* arg )
  {
  ...
  void KeyCache::forAllNamespaces ( void (*func) ( void* arg, NamespaceId nsId, const char* url ), void* arg )
  {
   ...
   * *********************** HTML Key functions **********************
   */
  bool KeyCache::isHTMLKeyIn ( KeyId keyId, const char** qnames )
  {
    if ( getNamespaceId(keyId) )
      return false;
  ...
  bool KeyCache::isHTMLKeyHTML ( KeyId keyId )
  {
    static const char* qnames[] = { "html", NULL };
    return isHTMLKeyIn(keyId, qnames);
  }

  bool KeyCache::isHTMLKeyHead ( KeyId keyId )
  {
    static const char* qnames[] = { "head", NULL };
    return isHTMLKeyIn(keyId, qnames);
  }

  bool KeyCache::isHTMLKeyDisableTextProtect ( KeyId keyId )
  {
    static const char* qnames[] = { "script", "style", NULL };
    return isHTMLKeyIn(keyId, qnames);
  }

  bool KeyCache::isHTMLKeyDisableAttributeValue ( KeyId keyId )
  {
    static const char* qnames[] = { "checked", "selected", NULL };
    return isHTMLKeyIn(keyId, qnames);
  }

  bool KeyCache::isHTMLKeySkipIndent ( KeyId keyId )
  {
    static const char* qnames[] = { "th", "td", "a", NULL };
    return isHTMLKeyIn(keyId, qnames);
  }

  bool KeyCache::isHTMLKeySkipMarkupClose ( KeyId keyId )
  {
    static const char* qnames[] = { "area", "base", "basefont", "br", "col", "frame", "hr", "img", "input",
        "isindex", "link", "meta", "param", NULL };
    return isHTMLKeyIn(keyId, qnames);
  }

  bool KeyCache::isHTMLKeyURIAttribute ( KeyId eltKeyId, KeyId attrKeyId )
  {
    ...
    static const QNamePair qnames[] =
    {
        ...
        { "area", "href" },
        { "link", "href" },
        { "img", "src" }, { "img", "longdesc" }, { "img", "usemap" }, { "img", "onclick" },
    ...
    if ( getNamespaceId(eltKeyId) ) return false;
    if ( getNamespaceId(attrKeyId) ) return false;
