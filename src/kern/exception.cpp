...
namespace Xem
{
    ...
    void
    Exception::doAppendMessageVA (const char* file, const char* function, int line, const char* format, va_list a_list)
    {
        if (silent)
            return;
    ...
    Exception::getMessage ()
    {
        ...
        for (MessageList::iterator iter = messages.begin(); iter != messages.end(); iter++)
        {
            ...
            writer.doPrintf("[%s:%s:%d] %s\n", fragment.file, fragment.function, fragment.line,
                            fragment.fragment.c_str());
        }
