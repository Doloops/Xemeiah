...
namespace Xem
{
  bool PersistentDocumentAllocator::forPageInfoPage ( PageInfoPage* pageInfoPage, RelativePagePtr offset, bool isStolen, 
        ...
        if ( willProcess )
          {
            ...
            (this->*action) ( offset, *pageInfo, PageType_Segment,
              isStolen || ( bridcmp ( getBranchRevId(), pageInfo->branchRevId ) != 0 ),
              arg );        
#if 0            
  ...
  bool PersistentDocumentAllocator::forIndirectionPages ( IndirectionPage* indirPage, __ui32 level, RelativePagePtr offset, bool isStolen,
    ...
    for ( __ui32 index = 0 ; index < Indirection_pointerNumber ; index++ )
      {
        ...
        if ( absPagePtr == NullPage ) continue;
        if ( __isStolen(absPagePtr) && ownedIndirOnly ) continue;
        ...
        if ( indirAction )
          {
            (this->*indirAction) ( offset, absPagePtr, __getPageType(absPagePtr), isStolen || ( __isStolen(absPagePtr) ), arg );
          }
  ...
  bool PersistentDocumentAllocator::forAllIndirectionPages ( IndirectionPageAction indirAction, SegmentPageAction segmentAction, void *arg, 
      bool ownedIndirOnly, bool ownedSegmentOnly )
  {
    Lock lock ( mapMutex );
    ...
    if ( !__isStolen ( getRevisionPage()->indirection.firstPage ) || !ownedIndirOnly )
      {
        ...
        if ( getRevisionPage()->indirection.firstPage && indirAction )
          (this->*indirAction) ( NullPtr, getRevisionPage()->indirection.firstPage, __getPageType(getRevisionPage()->indirection.firstPage),
              __isStolen ( getRevisionPage()->indirection.firstPage ),
              arg );
      }
