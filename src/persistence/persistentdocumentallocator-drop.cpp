...
namespace Xem
{
    ...
    void
    PersistentDocumentAllocator::drop ()
    {
        AssertBug(revisionPageRef.getPage(), "Null RevsionPage !\n");

        Warn("Dropping '%llx:%llx'\n", _brid(revisionPageRef.getPage()->branchRevId));

        AbsolutePagePtr revisionPagePtr = revisionPageRef.getPage()->revisionAbsolutePagePtr;
        AbsolutePagePtr lastRevisionPagePtr = revisionPageRef.getPage()->lastRevisionPage;
        ...
        if (branchPage->lastRevisionPage != revisionPagePtr)
        {
            throwException(Exception, "Could not drop revision '%llx:%llx' : "
                           ...
                           _brid(revisionPageRef.getPage()->branchRevId), branchPage->lastRevisionPage,
                           revisionPagePtr);
        }
    ...
    bool
    PersistentDocumentAllocator::dropOwnedIndirectionPage (RelativePagePtr relPagePtr, AbsolutePagePtr absPagePtr,
    ...
    bool
    PersistentDocumentAllocator::dropOwnedSegmentPage (RelativePagePtr relPagePtr, PageInfo& pageInfo,
                                                       PageType pageType, bool isStolen, void* arg)
    {
        DropStats* stats = (DropStats*) arg;
    ...
    bool
    PersistentDocumentAllocator::dropAllPages ()
    {
        AssertBug(revisionPageRef.getPage(), "Null RevsionPage !\n");
        ...
        {
            AbsolutePageRef<PageList> pageListRef = getPageList(revisionPageRef.getPage()->freePageList);
            for (__ui64 index = 0; index < pageListRef.getPage()->number; index++)
            {
                dropStats.dropMap[pageListRef.getPage()->pages[index]] = true;
            }
            // releasePage ( revisionPageRef.getPage()->freePageList );
            dropStats.dropMap[revisionPageRef.getPage()->freePageList] = true;
        }
#endif

        Info("Revision %llx:%llx has %llu owned pages, and %lu free pages:\n", _brid(getBranchRevId()),
             getRevisionPage()->ownedPages, (unsigned long ) dropStats.dropMap.size());
        ...
        __ui64 droppedPages = dropStats.droppedPages[PageType_Indirection] + dropStats.droppedPages[PageType_PageInfo]
        ...
        for (DropStats::DropMap::iterator iter = dropStats.dropMap.begin(); iter != dropStats.dropMap.end(); iter++)
        {
         ...
         */
        alterRevisionPage();
        memset(revisionPageRef.getPage(), 0, PageSize);
        protectRevisionPage();
