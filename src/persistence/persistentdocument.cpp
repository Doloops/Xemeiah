...
namespace Xem
{
    PersistentDocument::PersistentDocument (Store& __s, PersistentDocumentAllocator& persistentDocumentAllocator,
    ...
    void
    PersistentDocument::releaseDocumentResources ()
    {
    ...
    void
    PersistentDocument::scheduleBranchForRemoval ()
    {
        scheduleDocumentRelease();
    ...
    void
    PersistentDocument::scheduleDocumentRelease ()
    {
        scheduledDocumentRelease = true;
    }

    bool
    PersistentDocument::isScheduledRevisionRemoval ()
    {
        return scheduledRevisionRemoval;
    }

    bool
    PersistentDocument::mayIndex ()
    {
        return true;
    ...
    void
    PersistentDocument::merge (XProcessor& xproc, bool keepBranch)
    {
        if (keepBranch)
        {
        ...
        catch (Exception * e)
        {
            detailException(e, "Could not merge branch from [%llx:%llx] to [%llx:%llx]\n", _brid(getBranchRevId()),
                            _brid(mergeDocument->getBranchRevId()));
    ...
    void
    PersistentDocument::commit ()
    {
        Bug("Forbidden call to commit() : shall call commit(XProcessor&) !\n");
    }

    void
    PersistentDocument::commit (XProcessor& xproc)
    {
        getPersistentDocumentAllocator().commit();

        BranchFlags branchFlags = getBranchFlags();

        if ((branchFlags & BranchFlags_AutoForked) && (branchFlags & BranchFlags_AutoMerge))
        {
            merge(xproc, false);
            scheduleBranchForRemoval();
        }
    ...
    bool
    PersistentDocument::mayCommit ()
    {
        return getPersistentDocumentAllocator().mayCommit();
    }

    void
    PersistentDocument::dropAllPages ()
    {
    ...
    PersistentDocument::reopen ()
    {
        ...
        if (getDocumentOpeningFlags() == DocumentOpeningFlags_ExplicitRead)
        {
            throwException(Exception,
                           "reopenning a document which has been marked DocumentOpeningFlags_ExplicitRead !\n");
        }
        if (getBranchFlags() & BranchFlags_MayNotBeXUpdated)
        {
            throwException(Exception, "Document (tag=%s)(role=%s) [%llu:%llu], branch '%s' may not be XUpdated !\n",
                           getDocumentTag().c_str(), getRole().c_str(), _brid(getBranchRevId()),
                           getBranchName().c_str());
        }
