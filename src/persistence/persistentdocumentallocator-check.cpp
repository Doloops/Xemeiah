...
#define AssertError(__cond,...) if ( ! (__cond) ) CheckError ( __VA_ARGS__ )
  ...
  bool PersistentDocumentAllocator::checkIndirectionPage ( RelativePagePtr relPagePtr, AbsolutePagePtr absPagePtr, PageType pageType, bool isStolen, void* arg )
  {
  ...
  bool PersistentDocumentAllocator::checkSegmentPage ( RelativePagePtr relPagePtr, PageInfo& pageInfo, PageType pageType, bool isStolen, void* arg )
  {
    AssertBug ( pageType, "Invalid non-typed page !\n" );
    AbsolutePagePtr absPagePtr = pageInfo.absolutePagePtr;
    PersistentStore::AllocationStats* pStats = (PersistentStore::AllocationStats*) arg;

    if ( isStolen ) absPagePtr |= PageFlags_Stolen;
    if ( pageType == PageType_Revision )
      {
        return true;
      }
    pStats->referencePage ( "In Revision", revisionPage->branchRevId, relPagePtr,
        absPagePtr & PagePtr_Mask, pageType, __isStolen(absPagePtr) );
    return true;
  }
  
  bool PersistentDocumentAllocator::checkPages ( PersistentStore::AllocationStats& stats )
  {
    forAllIndirectionPages ( &PersistentDocumentAllocator::checkIndirectionPage, &PersistentDocumentAllocator::checkSegmentPage, &stats, false, false );
    Log_Check_Page ( "[After Check : alloc=%llx, map=%llx, chunkMapSize=%llx]\n",
        areasAlloced, areasMapped, getPersistentStore().getChunkMapSize() );
    return true;
  }
  
  bool PersistentDocumentAllocator::checkRelativePages ( PersistentStore::AllocationStats& stats )
  {
    ...
    for ( RelativePagePtr relPagePtr = PageSize ; relPagePtr < getDocumentAllocationHeader().nextRelativePagePtr ; relPagePtr+= PageSize )
      {
        ...
        AbsolutePagePtr myAbsPagePtr = NullPage; bool stolen = false;
  ...
  bool PersistentDocumentAllocator::checkContents ( )
  {
    CheckInfo ( "Checking revision contents for revisionPage=%p, revision=[%llx:%llx], nextRelativePagePtr=%llx, documentHeadPtr=%llx, "
        "indirection[firstPage=%llx, level=%x]\n",
    ...
    CheckInfo ( "Total pages per profile : (total=%llu, nbFreeListHeaders=%x)\n", totalPages, getDocumentAllocationHeader().nbAllocationProfiles );
    for ( AllocationProfile allocProfile = 0 ; allocProfile < getDocumentAllocationHeader().nbAllocationProfiles ; allocProfile++ )
      {
        if ( nbPagesPerAllocationProfile[allocProfile] )
          {
            CheckInfo ( "\tprofile %x : %llu pages (%llu %%).\n", allocProfile, nbPagesPerAllocationProfile[allocProfile],
    ...
    CheckInfo ( "\tContents : alloced %llu MBytes (0x%llx bytes) at %p for 0x%llx chunks (revision has %llx pages, %llu MBytes)\n",
    ...
    enum SegmentType
    {
      SegmentType_AttributeHead       = 'A',       SegmentType_Attribute       = 'a',
      SegmentType_BlobHead            = 'B',       SegmentType_Blob            = 'b',
      SegmentType_DocumentHeadHead    = 'D',       SegmentType_DocumentHead    = 'd',
      SegmentType_ElementHead         = 'E',       SegmentType_Element         = 'e', 
      SegmentType_FreePageHead        = 'P',       SegmentType_FreePage        = 'p',
      SegmentType_FreeSegmentHead     = 'F',       SegmentType_FreeSegment     = 'f',
      SegmentType_JournalItemHead     = 'J',       SegmentType_JournalItem     = 'j',      
      SegmentType_ReservedHead        = 'R',       SegmentType_Reserved        = 'r',
      SegmentType_SKMapItemHead       = 'I',       SegmentType_SKMapItem       = 'i',
      SegmentType_SKMapListHead       = 'L',       SegmentType_SKMapList       = 'l',
      SegmentType_TextualContentsHead = 'T',       SegmentType_TextualContents = 't'
    };
...
#define __dumpPageContents(__relPagePtr) \
  do { __ui64 index = __relPagePtr / chunkSize;  \
    ...
    CheckInfo ( "Identifying JournalItem : alloced[first=%llx,last=%llx], first=%llx\n", 
        getRevisionPage()->journalHead.firstAllocedJournalItem, getRevisionPage()->journalHead.lastAllocedJournalItem,
        ...
        if ( journalItem->nextJournalItem == NullPtr )
          {
            if ( journalItemPtr != getRevisionPage()->journalHead.lastAllocedJournalItem )
              {
                CheckError ( "Corrupted journal list end : last journal entry in revision is %llx, but list ends at %llx\n",
            ...
            for ( SegmentPtr holePtr = fslh->firstFreeHole[level] ; holePtr ; )
              {
                ...
                if ( freeSegment->dummy != FreeSegment_dummy )
                  {
                    CheckError ( "Invalid freeSegment ! profile=%x, level=%x, nbInChain=%llx, holePtr=%llx, stored in %llx, dummy=%x\n", 
                      profile, level, nbInChain, holePtr, holePagePtr,
                      freeSegment->dummy );
                ...
                if ( error )
                  {
                    CheckError ( "Invalid FreeSegment ! profile=%x, level=%x, nbInChain=%llx, holePtr=%llx, stored in %llx, dummy=%x, size=%llx, next=%llx\n", 
                      profile, level, nbInChain, holePtr, holePagePtr,
                      freeSegment->dummy, freeSegment->size, freeSegment->next );
                    erroneousPages[holePtr & PagePtr_Mask] = true;
                    if ( holePagePtr )
                        erroneousPages[holePagePtr] = true;
                    break;
    ...
    while ( true )
      {
        ...
        if ( eltSeg->flags & ElementFlag_HasAttributesAndChildren )
            ...
            for ( SegmentPtr attrPtr = eltSeg->attributesAndChildren.attrPtr ; attrPtr ; )
              {
                ...
                switch ( attrSeg->flag & AttributeType_Mask )
                  ...
                  case AttributeType_SKMap:
                    ...
                    break;
                  default:
                    CheckError ( "\tInvalid type attribute %x for attr=%llx , exiting attribute check for currentElt=%llx at %llx\n", 
            ...
            if ( eltSeg->attributesAndChildren.childPtr )
              {
                ...
                AssertError ( childSeg->fatherPtr == ptr, "Invalid father ptr from element=%llx, child=%llx, his father is =%llx\n",
                    ptr, eltSeg->attributesAndChildren.childPtr, childSeg->fatherPtr );
                AssertError ( childSeg->elderPtr == NullPtr, "Invalid elder ptr from element=%llx, child=%llx, his elder is=%llx\n",
                    ptr, eltSeg->attributesAndChildren.childPtr, childSeg->elderPtr );
        ...
        else if ( eltSeg->flags & ElementFlag_HasTextualContents )
          {
            if ( eltSeg->textualContents.size <= eltSeg->textualContents.shortFormatSize )
              ...
              {
                __markSegment ( SegmentType_TextualContents, eltSeg->textualContents.contentsPtr, eltSeg->textualContents.size );
              }
          }
        else
          {
        ...
        while ( ! eltSeg->youngerPtr )
          {
            ...
            if ( ptr == documentHead->rootElementPtr ) break;
            if ( ptr == documentHead->metaElementPtr ) break;
            
            if ( ! ptr ) { eltSeg = NULL; break; }
        ...
        AssertError ( youngerSeg->fatherPtr == eltSeg->fatherPtr, 
            ...
            eltSeg->fatherPtr, youngerSeg->fatherPtr );
        ...
        if ( attributesPerType[type] )
          {
            CheckInfo ( "\tAttributeType %x : %llu attributes, total %llu bytes, avg %llu bytes per attr, max %llu\n", 
               type, attributesPerType[type],
               attributeSizesPerType[type], attributeSizesPerType[type] / attributesPerType[type],
               attributeMaxSizesPerType[type] ); 
          }
      }
    ...
    if ( identifiedSize != nextRelativePagePtr )
      {
        CheckError ( "Identified size=%llu bytes (0x%llx) on %llu (0x%llx) allocated -> %llu%% found.\n", 
          identifiedSize, identifiedSize, 
          nextRelativePagePtr, nextRelativePagePtr,
    ...
    if ( erroneousPages.size() )
      {
        ...
        for ( std::map<RelativePagePtr,bool>::iterator iter = erroneousPages.begin();
          iter != erroneousPages.end() ; iter++ )
          {
            __ui64 relPagePtr = iter->first;

            PageInfo pageInfo;
            
            if ( ! getPageInfo ( relPagePtr, pageInfo ) )
              {
            ...
            else
              {
                ...
                CheckError ( "\tErroneous page %llx, absPagePtr=%llx, pageFlags=%llx\n", 
                    relPagePtr, absPagePtr, pageFlags );
#if 0
        ...
        if ( chunkType != '?' )
          {
            ...
            if ( isInUnknown )
              {
                CheckError ( "Unknown area [%08llx,%08llx] size=%08llx\n", unknownChunkStart * chunkSize, idx*chunkSize, (idx-unknownChunkStart) * chunkSize );
                nbUnknownSegments++;
                isInUnknown = false;
    ...
    for ( __ui32 level = 0 ; level < FreeSegmentsLevelHeader::levelNumber ; level++ )
      {
        if ( nbUnknownPerLevel[level] == 0 ) continue;
        CheckError ( "\tUnknown per level : level=%x, number=%llu (0x%llx)\n",
          level, nbUnknownPerLevel[level], nbUnknownPerLevel[level] );
      }
    ...
    for ( unsigned char typeHead = 'A' ; typeHead <= 'Z' ; typeHead++ )
      {
        ...
        if ( nbChunksPerType[typeHead] ) 
          {
            CheckInfo ( "typeHead %c : %llu (0x%llx) chunks\n", typeHead, nbChunksPerType[typeHead], nbChunksPerType[typeHead] );
          }
