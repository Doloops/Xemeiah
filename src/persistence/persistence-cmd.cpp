...
#define __showSize(_x) Info ( "\tsizeof(%s) : %lu bytes (0x%lx)\n", \
  ...
  XemRunFileImport(PersistentStore& store, const char* filePath)
  {
    ...
    if (!branchId)
      {
        branchId = store.getBranchManager().createBranch(branchName,
            BranchFlags_None);
      }
    ...
    Document* targetDocument = store.getBranchManager().openDocument(branchId,
        0, DocumentOpeningFlags_Write);
      {
  ...
  XemRunDump(PersistentStore& store)
  {
    ...
    BranchId branchId = store.getBranchManager().getBranchId(branchName);
    Document* document = store.getBranchManager().openDocument(branchId, 0,
        DocumentOpeningFlags_Read);
  ...
  XemRunPersistenceAction(int argc, char** argv)
  {
    ...
    if (argc < 3)
      {
        fprintf(stderr,
            "Not enought parameters given : must provide at least the action to perform.\n");
        goto print_usage;
      }
            ...
            if (strncmp(arg, "--", 2) == 0)
            ...
            else if (strcmp(arg, "dump-branches") == 0)
              {
                ...
                Document* branchesDocument =
                    store.getBranchManager().generateBranchesTree(xproc);
                branchesDocument->getRootElement().toXML(1,
                    ElementRef::Flag_ChildrenOnly | ElementRef::Flag_XMLHeader);
