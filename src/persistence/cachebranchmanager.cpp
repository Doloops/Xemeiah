...
namespace Xem
{
  ...
  bool CacheBranchManager::isLocked ()
  {
    return branchManagerMutex.isLocked();
  }

  void CacheBranchManager::assertIsLocked ()
  {
    branchManagerMutex.assertLocked();
  }

  void CacheBranchManager::assertIsUnlocked ()
  {
    branchManagerMutex.assertUnlocked();
  }

  void CacheBranchManager::lockBranchManager ()
  {
    branchManagerMutex.lock();
  }

  void CacheBranchManager::unlockBranchManager ()
  {
    branchManagerMutex.unlock();
  }

  void CacheBranchManager::assertBranchLockedForWrite ( BranchId branchId )
  {
    AssertBug ( isBranchLockedForWrite(branchId), "Not locked for write !\n" );
  }

  void CacheBranchManager::assertBranchUnlockedForWrite ( BranchId branchId )
  {
    AssertBug ( !isBranchLockedForWrite(branchId), "Locked for write !\n" );
  }

  bool CacheBranchManager::isBranchLockedForWrite ( BranchId branchId )
  {
    if ( !branchId )
      {
  ...
  void CacheBranchManager::lockBranchForWrite ( BranchId branchId )
  {
    BranchInfo* branchInfo = branchMap[branchId];
    AssertBug ( branchInfo, "Invalid branchId.\n" );
  ...
  void CacheBranchManager::unlockBranchForWrite ( BranchId branchId )
  {
    BranchInfo* branchInfo = branchMap[branchId];
    AssertBug ( branchInfo, "Invalid branchId.\n" );
  ...
  CacheBranchManager::BranchInfo* CacheBranchManager::referenceBranch ( BranchId branchId, const BranchRevId& forkedFrom )
  {
  ...
  void CacheBranchManager::markBranchDependency ( BranchInfo* branchInfo )
  {
  ...
  void CacheBranchManager::checkBranchRemoval ( BranchInfo* branchInfo )
  {
  ...
  Document* CacheBranchManager::getDocumentFromCache ( BranchId branchId, RevisionId revisionId, DocumentOpeningFlags flags, RoleId roleId )
  {
    BranchInfo* branchInfo = branchMap[branchId];
  ...
  Document* CacheBranchManager::getDocumentFromCache ( BranchInfo* branchInfo, RevisionId revisionId, DocumentOpeningFlags flags, RoleId roleId )
  {
  ...
  void CacheBranchManager::releaseDocument ( Document* doc )
  {
    assertIsLocked();
    ...
    if ( doc->isScheduledRevisionRemoval() )
    ...
    else
      {
        ...
        if ( flags == DocumentOpeningFlags_ExplicitRead
            || flags == DocumentOpeningFlags_Read
            || flags == DocumentOpeningFlags_FollowBranch
  ...
  void CacheBranchManager::updateDocumentReference ( Document* doc, BranchRevId oldBrId, BranchRevId newBrId ) 
  { 
    Warn ( "[UPDATEDOCREF] doc=%p (%llx:%llx) oldBrId=[%llx:%llx] => newBrId=[%llx:%llx]\n",
        doc, _brid(doc->getBranchRevId()), _brid(oldBrId), _brid(newBrId) );
    lockBranchManager ();
  ...
  void CacheBranchManager::garbageCollectorThread ( )
  {
    setStarted();
    int loopInterval = 1;
...
#if 0
  void CacheBranchManager::deleteDocument ( Document* doc )
  {
    Warn ( "[DELETE DOC] Deleting document at %p\n", doc );
    Bug ( "." );
    // delete ( doc );
  }
#endif
