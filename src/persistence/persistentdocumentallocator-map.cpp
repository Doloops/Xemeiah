...
namespace Xem
{
  void PersistentDocumentAllocator::doInitialize ( AbsolutePagePtr revisionPagePtr )
  {
  ...
  void PersistentDocumentAllocator::allocateAreaMap ( __ui64 areaIdx )
  {
  ...
  void* PersistentDocumentAllocator::mapAreaFromFile ( __ui64 areaIdx, AbsolutePagePtr offset )
  {
  ...
  void PersistentDocumentAllocator::mapArea ( __ui64 areaIdx )
  {
    lockMutex_Map();
  ...
  SegmentPage* PersistentDocumentAllocator::getRelativePage ( RelativePagePtr relPagePtr )
  {
    mapMutex.assertLocked();
  ...
  void PersistentDocumentAllocator::doMapArea ( __ui64 areaIdx )
  {
    mapMutex.assertLocked();
    ...
    for ( PageInfoIterator iter(*this, beginRelPagePtr ) ; iter.first() <= endRelPagePtr ; iter++ )
      {
        ...
        AssertBug ( absPagePtr, "Null absolute page ptr provided for relPagePtr=0x%llx, nextRelativePagePtr=0x%llx !\n", 
  ...
  void* PersistentDocumentAllocator::getArea ( __ui64 areaIdx )
  {
  ...
  void PersistentDocumentAllocator::mapRelativePages ( void* area, RelativePagePtr relPagePtr, AbsolutePagePtr absPagePtr, __ui64 count )
  {
    mapMutex.assertLocked();
#if PARANOID
    AssertBug ( relPagePtr % PageSize == 0, "RelativePagePtr not aligned to PageSize !\n" );
    ...
    int res = remap_file_pages ( page, PageSize * count, 
        0, pgoff, 
        0 | ( MAP_SHARED | MAP_POPULATE | MAP_NONBLOCK ) 
        );
    if ( res == -1 )
      {
        Bug ( "Could not remap file page : errno=%d:%s. Arguments : page=%p, size=%llx, pgoff=%lx\n",
              errno, strerror ( errno ),
              page, (long long) PageSize*count, (long) pgoff );
  ...
  void PersistentDocumentAllocator::doMapAllPages ()
  {
