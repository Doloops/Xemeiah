...
namespace Xem
{
  ...
  void PersistentStore::initPersistentStore ()
  {
    readOnly = false;
    superBlock = NULL;
    fd = -1;
  ...
  : chunkMapMutex("ChunkMap"),
  superblockMutex("SuperBlock"),
  freePageHeaderMutex("FreePageHeader")
  {
    initPersistentStore ();
  }
  ...
  PersistentBranchManager& PersistentStore::getPersistentBranchManager ()
  {
    return *branchManager;
  }

  BranchManager& PersistentStore::getBranchManager ()
  {
    return getPersistentBranchManager();
  }

  bool PersistentStore::reserveElementIds ( ElementId& nextId, ElementId& lastId )
  {
    lockSB ();
  ...
  bool PersistentStore::isFreePageCacheFull ()
  {
  ...
  bool PersistentStore::setReadOnly ( bool __readOnly )
  { 
  ...
  bool PersistentStore::isReadOnly () const
  {
    return readOnly;
  }

  bool PersistentStore::openFile ( const char* _filename, bool mayCreate )
  {
    ...
    Info ( "fileLength of '%s' is %llu (0x%llx) bytes, %llu (0x%llx) pages.\n", 
	   filename, (__ui64) fileLength, (__ui64) fileLength,
  ...
  bool PersistentStore::extendFile ( __ui64 expectedFileLength )
  {
    AssertBug ( fileLength % PageSize == 0, "File length is not a multiple of pageSize !\n" );
    ...
    if ( res == -1 )
      {
        Fatal ( "Could not extend file from %llu Mb to %llu Mb using ftruncate(fd=%d, length=%llu) : err=%d:%s\n", 
            fileLength>>20, expectedFileLength>>20, 
            fd, (__ui64) expectedFileLength,
            errno, strerror(errno) );
        return false; 
      }
    Log ( "Extended file : from 0x%llx (%llu Mb) to 0x%llx (%llu Mb)\n",
      fileLength, fileLength >> 20, expectedFileLength, expectedFileLength >> 20 );
    fileLength = expectedFileLength;
    return true;
  }

  bool PersistentStore::closeFile ()
  {
  ...
  bool PersistentStore::format ( const char* filename )
  {
    Info ( "Formatting Store File '%s' ...\n", filename );
    if ( ! openFile ( filename, true ) )  return false;
    SuperBlock * sb = getSB();
  ...
  bool PersistentStore::open ( const char* filename )
  {
    Info ( "Openning Store File '%s'\n", filename );
    if ( ! openFile ( filename, false ) ) return false;
    if ( ! checkFormat () ) { closeFile(); return false; }
#if 0
    if ( ! readOnly )
      {
        if ( ! dropUncommittedRevisions() ) { closeFile(); return false; }
      }
#endif
    ...
    Info ( "Openned file '%s' : noMansLand at=0x%llx, nbFreePages=0x%llx, fileLength=%lu MBytes\n", 
        filename, getSB()->noMansLand, getSB()->nbFreePages, (unsigned long) fileLength >> 20 );
  ...
  void PersistentStore::housewife ()
  {
