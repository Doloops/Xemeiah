...
namespace Xem
{
  ...
  void XPathParser::init ( ElementRef& eltRef, AttributeRef& attrRef, bool isAVT )
  {
  ...
  void XPathParser::clearInMem ()
  {
    ...
    if ( resourceBlock ) free ( resourceBlock );
    if ( packedSegment ) free ( packedSegment );
  ...
  bool XPathParser::isNumeric ( const char* expr )
  {
    ...
    if ( ( '0' <= *expr && *expr <= '9' ) || *expr == '-' || *expr == '.' )
    {
        ...
        for ( const char* c = &(expr[1]) ; *c ; c++ )
          {
            if ( '0' <= *c && *c <= '9' ) continue;
            if ( ! hasDot && *c == '.' )  { hasDot = true; continue; }
            return false;
          }
        return true;
    }
  ...
  KeyId XPathParser::getKeyId ( const String& key )
  {
  ...
  XPathStepId XPathParser::allocStep ()
  {
  ...
  XPathStep* XPathParser::getStep ( XPathStepId stepId )
  {
  ...
  XPathStepId XPathParser::allocStepResource ( const char* text )
  {
  ...
  void XPathParser::parse ( const char* expr, bool embedded )
  {
    if ( ! expr || ! expr[0] )
      {
  ...
  XPathStepId XPathParser::doParse ( const char* xpath )
  {
    /*
   ...
   */
  XPathStepId XPathParser::tokenToStep ( Token* token )
  {
    ...
    else
      {
        ...
        case '$':
          {
            XPathStepId stepId = allocStep ();
            XPathStep* step = getStep ( stepId );
            AssertInvalid ( token->subExpression->isQName() && token->subExpression->next == NULL, "Invalid variable contents.\n" );
        ...
        case '|':
          {
            AssertInvalid ( ! token->next, "Invalid token '%c' (%s) with next '%c' (%s) (at %p, next=%p)!\n", 
                token->symbol, token->token.c_str(),
                token->next->symbol, token->next->token.c_str(), 
                token, token->next );
            XPathStepId stepId = allocStep ();
            XPathStep* step = getStep ( stepId );
                    ...
                    if ( next->symbol == '[' )
                      {
                        ...
                        predicateId ++;
                        AssertInvalid ( predicateId < XPathStep_MaxPredicates, "Maximum number of predicates reached !\n" );
                      }
        ...
        case '#':
           ...
           */
          {
            ...
            AssertInvalid ( token->next->next && token->next->next->isSymbol() && token->next->next->symbol == '(',
            ...
            AssertInvalid ( funcStep->action == XPathFunc_FunctionCall, "Operator following '->' did not resolve as a function !\n" );
  ...
  void XPathParser::tokenToFunctionStep ( XPathStep* step, Token* token )
  {
            ...
            if ( token->next->subExpression->isText() )
              {
                ...
                if ( token->next->subExpression->token == "*" )
                  ...
                  {
                    localKeyId = parsingFromKeyCache->getKeyId ( 0, 
                        token->next->subExpression->token.c_str(), true ); 
                  }
...
#undef __XPath_Func
    ...
    for ( std::list<Token*>::iterator iter = arguments.begin() ; iter != arguments.end() ; iter++ )
    {
        ...
        Token* arg = *iter;
        if ( arg->isText() && ( 
              ( i == 1 && ( step->action == XPathFunc_Key 
                      || step->action == XPathFunc_SystemProperty 
  ...
  void XPathParser::qnameTokenToTest ( XPathStep* step, Token* token )
  {
    if ( token->token == "." )
      {
...
#undef __XPath_Axis
  ...
  void XPathParser::nextTokenToStep ( XPathStep* step, Token* token, Token* next )
  {
    if ( ! next )
        return;
    if ( ! next->isSymbol() )
...
#define __XPATHPARSER_IS_EQUALS_ATTRIBUTE_VARIABLE(__token) \
      ...
      && !((__token)->subExpression2->next) )
                ...
                if ( predicateId == 0 && step->action == XPathAxis_Child 
                  && KeyCache::getLocalKeyId ( step->keyId )
                  && (!nextPredicate->next || nextPredicate->next->symbol != '[' )
                  && ( __XPATHPARSER_IS_EQUALS_ATTRIBUTE_VARIABLE ( nextPredicate->subExpression )
                    || ( nextPredicate->subExpression->symbol == 'B' && nextPredicate->subExpression->token == "and"
                      && ( __XPATHPARSER_IS_EQUALS_ATTRIBUTE_VARIABLE ( nextPredicate->subExpression->subExpression ) 
                        || ( nextPredicate->subExpression->subExpression->symbol == 'B' && nextPredicate->subExpression->subExpression->token == "and"
                        && __XPATHPARSER_IS_EQUALS_ATTRIBUTE_VARIABLE ( nextPredicate->subExpression->subExpression->subExpression )  )
                         )
                       )
                   ...
                   )
                  {
                    ...
                    else if ( nextPredicate->subExpression->symbol == 'B' && nextPredicate->subExpression->token == "and" )
                      {
                    ...
                    sprintf ( synthetizedKey, "__synth_childLookup_%x_%x",
                        step->keyId, getStep(step->functionArguments[1])->keyId );
  ...
  void XPathParser::binaryTokenToStep ( XPathStep* step, Token* token )
  {
    step->action = XPathAction_NULL;
    switch ( token->symbol )
    {
    case '+': step->action = XPathFunc_Plus; break;
    case '*': step->action = XPathFunc_Multiply; break;
    case '=': 
      {
#ifdef __XEM_XPATHPARSER_OPTIMIZE_LOCALNAME_EQUAL
        if ( token->subExpression->isQName()
          && token->subExpression->token == "local-name"
    ...
    case '!': step->action = XPathComparator_NotEquals; break;
    case '|': step->action = XPathFunc_Union; break;
    case 'B':
        if ( token->token == "div" ) step->action = XPathFunc_Div;
        else if ( token->token == "mod" ) step->action = XPathFunc_Modulo;
        else if ( token->token == "and" ) step->action = XPathFunc_BooleanAnd;
        else if ( token->token == "or" ) step->action = XPathFunc_BooleanOr;
        else { Invalid ( "Invalid token '%s'\n", token->token.c_str() ); }
        break;
    case '-': 
        step->action = XPathFunc_Minus;
        break;
    case '<':
        if ( token->token == "<=" ) step->action = XPathComparator_LessThanOrEquals;
        else step->action = XPathComparator_LessThan;
        break;
    case '>':
        if ( token->token == ">=" ) step->action = XPathComparator_GreaterThanOrEquals;
        else step->action = XPathComparator_GreaterThan;
        break;
    default:
  ...
  void XPathParser::checkPositionnalExpression ( XPathStep* step, XPathStepFlags& flags )
  {
    ...
    if ( __XPathAction_isFunction(step->action) || __XPathAction_isComparator(step->action) )
      {
        for ( int argIndex = 0 ; argIndex < XPathStep_MaxFuncCallArgs ; argIndex++ )
          {
            if ( step->functionArguments[argIndex] == XPathStepId_NULL ) break;
  ...
  void XPathParser::tagStepPredicatesFlags ( XPathStep* step )
  {
  ...
  void XPathParser::computeLastSteps ()
  {
        ...
        if ( step->nextStep != XPathStepId_NULL )
          {
            ...
            AssertBug ( next->lastStep == XPathStepId_NULL,
              "Last step already set !\n" );
            ...
            if ( step->action == XPathAxis_Root
               && ( next->action == XPathAxis_Descendant
               || next->action == XPathAxis_Descendant_Or_Self ) )
              {
