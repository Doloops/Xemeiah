...
namespace Xem
{
  ...
  SAXHandlerDom::SAXHandlerDom ( XProcessor& _xproc, ElementRef& __element )
    : xproc(_xproc), rootElement ( __element ), currentElement ( __element ),
      namespaceAlias ( __element.getKeyCache() )
  {
  ...
  KeyCache& SAXHandlerDom::getKeyCache()
  {
    return xproc.getKeyCache();
  }

  bool SAXHandlerDom::setKeepTextMode ( const String& mode )
  {
    if ( mode == "all" )         keepTextMode = KeepTextMode_All;
    else if ( mode == "normal" ) keepTextMode = KeepTextMode_All;
    else if ( mode == "xsl" )
      {
        keepTextMode = KeepTextMode_XSL;
        xslTextKeyId = getKeyCache().getKeyId(
            getKeyCache().getNamespaceId("http://www.w3.org/1999/XSL/Transform"),
            "text", true
          );

      }
    else if ( mode == "none" )   keepTextMode = KeepTextMode_None;
    else
      {
  ...
  bool SAXHandlerDom::setKeepTextAtRootElement ( bool value )
  {
  ...
  inline SAXHandlerDom::ParsedAttribute* SAXHandlerDom::getNewParsedAttribute()
  {
  ...
  void SAXHandlerDom::dumpContext ( Exception* e )
  {
    for ( ElementRef parent = currentElement ;
          parent ; parent = parent.getFather() )
      {
        detailException ( e, "\tAt Element : '%s'\n", parent.getKey().c_str() );
        for ( AttributeRef attr = parent.getFirstAttr() ;
  ...
  void SAXHandlerDom::eventElement ( const char* ns, const char *name )
  {
  ...
  void SAXHandlerDom::eventAttr ( const char* ns, const char *name, const char *value )
  {
        ...
        if ( !elementPrefixId )
          {
             ...
             */
            KeyId keyId = getKeyCache().getKeyId ( targetNamespaceId, getKeyCache().getLocalKeyId(currentElement.getKeyId()) );
  ...
  void SAXHandlerDom::eventAttrEnd ()
  {
    ...
    for ( __ui32 parsedAttributes_index = 0 ; parsedAttributes_index < parsedAttributes_number ; parsedAttributes_index++ )
      {
  ...
  void SAXHandlerDom::eventElementEnd ( const char* ns, const char* name )
  {
        ...
        if ( keyId != currentElement.getKeyId() )
          {
            ...
            detailException ( e, "End element did not match ! I am on %x:'%s', parser tells %x:'%s:%s'\n", 
                ...
                currentElement.getKeyCache().getKey(namespaceAlias, currentElement.getKeyId()), 
                keyId, ns ? ns : "", name );
            dumpContext(e);
            throw ( e );
          }
  ...
  inline bool string_isspace ( const char* text )
  {
    bool sp = true;
  ...
  void SAXHandlerDom::eventText ( const char *text )
  {
    Log_SHD ( "eventText [%s]\n", text );
...
eventText_ImportSpacedText:
  ...
  void SAXHandlerDom::eventComment ( const char *comment )
  {
  ...
  void SAXHandlerDom::eventProcessingInstruction ( const char* name, const char * contents )
  {
  ...
  void SAXHandlerDom::eventEntity ( const char* entityName, const char* entityValue )
  {
    Log_SHD ( "ENTITY : '%s' = '%s'\n", entityName, entityValue );
    currentElement.getDocument().setUnparsedEntity(entityName, entityValue);
  }
  
  void SAXHandlerDom::eventNDataEntity ( const char* entityName, const char* ndata )
  {
    Log_SHD ( "ENTITY NDATA : '%s' = '%s'\n", entityName, ndata );
  }

  void SAXHandlerDom::eventDoctypeMarkupDecl ( const char* markupName, const char* value )
  {
        ...
        while ( tokens.size() )
          {
            ...
            String attrValue;
            if ( defaultDecl == "#FIXED" || (tokens.size() &&
                ( tokens.front().at(0) == '"' || tokens.front().at(0) == '\'' ) ) )
              {
  ...
  void SAXHandlerDom::parsingFinished ()
  {
