    ...
    template<typename KeyType>
        ...
        public:
            ...
            get (const char* keyName)
            {
                ...
                for (HashEntry* entry = hashEntries[hash]; entry; entry = entry->next)
                {
            ...
            INLINE
            void
            put (KeyType keyId, const char* keyName)
            {
                KeyHash hash = getHash(keyName);
            ...
            INLINE
            void
            remove (KeyType keyId, const char* keyName)
            {
                KeyHash hash = getHash(keyName);
                HashEntry* last = NULL;
                for (HashEntry* entry = hashEntries[hash]; entry; entry = entry->next)
