...
{
    int
    stringComparator (const char* left, const char* right, bool caseOrderUpperFirst);
...
#define String_ShowStringStats(...)
    ...
    public:
        ...
        inline
        int
        compareTo (const String& right, bool caseOrderUpperFirst = false)
        {
            if (!c_str())
                return -1;
            if (!right.c_str())
...
#define stringPrintf(__s,...) \
  do { \
    ...
    inline String
    StringFormatVA (const char* fmt, va_list a_list)
    {
        char* buf = (char*)malloc(512);
        vsnprintf(buf, 512, fmt, a_list);
        return stringFromAllocedStr(buf);
    }
    ...
    StringFormat (const char* fmt, ...)
    {
        va_list a_list;
        va_start(a_list, fmt);
        String result = StringFormatVA(fmt, a_list);
        va_end(a_list);
        return result;
    }
}
