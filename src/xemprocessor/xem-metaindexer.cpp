...
namespace Xem
{
            ...
            ElementRef(element), xemProcessor(_xemProcessor), xem(xemProcessor.xem)
    {
        if (*this)
        {
            AssertBug(getKeyId() == getKeyCache().getBuiltinKeys().xemint.dom_event(),
                      "Element is not a dom-event !\n");
            AssertBug(getAttrAsKeyId(xem.class_()) == xem.meta_indexer(), "Wrong class !\n");
        }
    }

    void
    MetaIndexer::eval (XProcessor& xproc, NodeSet& result, ElementRef& baseElement, NodeSet& args)
    {
    ...
    void
    MetaIndexer::build (XProcessor& xproc)
    {
        Log_DMI("build : this=%s, match=%s, use=%s, scope=%s\n", generateVersatileXPath().c_str(),
                getAttr(getKeyCache().getBuiltinKeys().xemint.match()).c_str(), getAttr(xem.use()).c_str(), getAttr(xem.scope()).c_str());
    ...
    ElementMultiMapRef
    MetaIndexer::getScope (XProcessor& xproc, NodeRef& base, bool create)
    {
        XPath scopeXPath(xproc, *this, xem.scope());
...
//      XPathParser& matchXPathParser, XPathParser& useXPathParser, XPathParser& scopeXPathParser )
    MetaIndexer
    XemProcessor::addMetaIndexer (Document& doc, ElementRef& declaration)
    {
        KeyId keyId = declaration.getAttrAsKeyId(xem.name());

        Log_DMI("call addMetaIndexer for elt=%s\n", declaration.generateVersatileXPath().c_str());
        ...
        domEvent.setMatchXPath(getXProcessor(), matchExpression);
    ...
    void
    XemProcessor::xemInstructionSetMetaIndexer ( __XProcHandlerArgs__)
    {
    ...
    void
    XemProcessor::domMetaIndexerSort (ElementRef elt, XPath& orderBy)
    {
    ...
    void
    XemProcessor::domMetaIndexerTrigger (DomEventType domEventType, ElementRef& domEventElement, NodeRef& nodeRef)
    {
    ...
    MetaIndexer
    XemProcessor::getMetaIndexer (Document& doc, KeyId keyId)
    {
    ...
    Log_DMI ( "Searching meta-indexer %s (%x)\n", getKeyCache().dumpKey(keyId).c_str(), keyId );
    ...
    for ( ChildIterator child (events); child; child ++ )
    {
        ...
        Log_DMI ( "Class : %s, name=%s (%x)\n", getKeyCache().dumpKey(child.getAttrAsKeyId(xem.class_())).c_str(),
                getKeyCache().dumpKey(child.getAttrAsKeyId(xem.name())).c_str(), child.getAttrAsKeyId(xem.name()));
