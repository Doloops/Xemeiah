...
namespace Xem
{
    ...
    Integer
    ElementRef::getLastPosition ()
    {
        // Warning, we may check against the rootElement() here !
        if (getFather())
            return getFather().getNumberOfChildren();
        return 1;
    }

    Integer
    ElementRef::getNumberOfChildren ()
    {
        Integer pos = 0;
    ...
    void
    ElementRef::rename (KeyId newKeyId)
    {
     ...
     */
    void
    ElementRef::insertBefore (ElementRef& elementRef) // Stub
    {
        AssertBug(getFather(), "Could not add before on root element !\n");
        if (!getElder())
            getFather().insertChild(elementRef, false);
        else
            getElder().insertAfter(elementRef, false);
        getDocument().appendJournal(*this, JournalOperation_InsertBefore, elementRef, 0);
    }

    void
    ElementRef::appendLastChild (ElementRef& elementRef) // Stub
    {
        ...
        ElementSegment* newChild = getDocumentAllocator().getSegment<ElementSegment, Write>(newChildPtr,
        ...
        if (me->attributesAndChildren.lastChildPtr)
        {
            ...
            ElementSegment* lastChild = getDocumentAllocator().getSegment<ElementSegment, Write>(
    ...
    void
    ElementRef::insertAfter (ElementRef& elementRef, bool updateJournal)
    {
#if PARANOID
        ...
        if (me->youngerPtr)
            newYounger->youngerPtr = me->youngerPtr;
    ...
    void
    ElementRef::insertChild (ElementRef& elementRef, bool updateJournal)
    {
        AssertBug(getElementPtr(), "Null Pointer given !!!\n");
        ...
        if (oldChildPtr)
            newChild->youngerPtr = oldChildPtr;
        getDocumentAllocator().protect(newChild);
     ...
     */
    void
    ElementRef::deleteElement ()
    {
        Bug("This is deprecated !\n");
    }

    void
    ElementRef::unlinkElementFromFather ()
    {
        ElementSegment* me = getMe<Write>();
    ...
    void
    ElementRef::deleteChildren (XProcessor& xproc)
    {
        ElementSegment* me = getMe<Write>();
...
//            ElementRef child ( getDocument(), childPtr );
    ...
    void
    ElementRef::deleteElement (XProcessor& xproc)
    {
        AssertBug(getElementPtr(), "Null Pointer given !!!\n");
        ...
        Log("deleteElement(ptr=%llx, me=%llx, flags=%x)\n", getElementPtr(), me->id, me->flags);
        ...
        else if (me->flags & ElementFlag_HasAttributesAndChildren)
        {
            /*
             * Trigger the eventDeleteElement
             */
        ...
        {
            Bug("Invalid flag for Element : (ptr=%llx, me=%llx, flags=%x)\n", getElementPtr(), me->id, me->flags);
        }
    ...
    void
    ElementRef::appendText (const String& text)
    {
    ...
    void
    ElementRef::copyContents (XProcessor& xproc, ElementRef& eRef, bool onlyBaseAttributes)
    {
    ...
    bool
    ElementRef::mustSkipWhitespace (XProcessor& xproc)
    {
        if (!isText())
            return false;
        if (!isWhitespace())
    ...
    String
    ElementRef::generateVersatileXPath ()
    {
