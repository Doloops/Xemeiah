...
namespace Xem
{
  ...
  void DomEvent::setHandlerId ( KeyId handlerId )
  {
  ...
  KeyId DomEvent::getHandlerId ( )
  {
    return getAttrAsKeyId(__xemint.handler());
  }

  void DomEvent::setEventMask ( DomEventMask domEventMask )
  {
  ...
  DomEventMask DomEvent::getEventMask ()
  {
    DomEventMask mask;
...
#else
  void DomEvent::addNodeQName ( KeyId attributeKeyId, KeyId qnameId )
  {
    addQNameInQNameList(attributeKeyId, qnameId);
  }
#endif

  void DomEvent::addElementQName ( KeyId keyId )
  {
    return addNodeQName ( __xemint.element_qnames(), keyId );
  }

  void DomEvent::addAttributeQName ( KeyId keyId )
  {
    return addNodeQName ( __xemint.attribute_qnames(), keyId );
  }


  void DomEvent::addQNames ( XPath& xpath )
  {
  ...
  void DomEvent::setMatchXPath ( XProcessor& xproc, const String& expression )
  {
  ...
  void DomEvent::addQName ( KeyId qnameId )
  {
  ...
  DomEvent DomEvents::createDomEvent ()
  {
  ...
  DomEvent DomEvents::registerEvent ( DomEventMask domEventMask, XPath& matchXPath, KeyId handlerId )
  {
  ...
  DomEvent DomEvents::registerEvent ( XPath& matchXPath, KeyId handlerId )
  {
    return registerEvent(DomEventMask_Element|DomEventMask_Attribute,matchXPath,handlerId);
  }

  DomEvent DomEvents::registerEvent ( DomEventMask domEventMask, KeyId matchKeyId, KeyId handlerId )
  {
    DomEvent domEvent = createDomEvent();
    domEvent.setEventMask(domEventMask);
    domEvent.setHandlerId(handlerId);
  ...
  SKMapHash DomEvents::hashEvent ( DomEventType domEventType, KeyId qnameId )
  {
    return (((SKMapHash) domEventType) << 32) + ((SKMapHash)qnameId);
  }

  void DomEvents::buildEventMap ( ElementMultiMapRef& eventMap, DomEvent& event, DomEventMask mask, KeyId qnameId )
  {
    for ( int i = 0 ; i < 32 ; i++ )
      {
        DomEventType type = (1<<i);
        if ( !mask.hasType(type) ) continue;
  ...
  void DomEvents::buildEventMap ( ElementMultiMapRef& eventMap, DomEvent& event, DomEventMask mask, QNameListRef& qnames )
  {
    if ( ! qnames ) return;
  ...
  void DomEvents::processEvent ( XProcessor& xproc, DomEventType domEventType, NodeRef& nodeRef )
  {
